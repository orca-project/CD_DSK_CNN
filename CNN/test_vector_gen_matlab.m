function [result_filename_I,result_filename_Q,result_filename_busy_flag_bin] = test_vector_gen_matlab(mat_filename, num_slot, slot_len)
...
CleanSig = zeros(slot_len, num_slot);
...
for i=1:num_slot
    ...
    sp = randi(1e6*64-slot_len_new+1);
    CleanSig(:,i) = signal_slot;
    s_test_vector(:,i) = signal_slot + noise_slot + interfere(sp:16:(sp+slot_len_new-1))*busy_flag(i);
end

format long g
STD_Ref= std(abs(CleanSig(:)))
Mean_Ref= mean(abs(CleanSig(:)))
STD_I_Ref = std(abs(real(CleanSig(:))))
STD_Q_Ref = std(abs(imag(CleanSig(:))))
